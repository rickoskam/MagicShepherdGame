...
public class WoestijnScript : MonoBehaviour {
	...
	void generateTerrain(){
		...
		for (int i = 0; i<aantalBomen; i++) {
			...
			if (watercheck (waterx, waterz, waterr, x, z) == 2 && Random.value < 0.01f) {
				if (Random.value > 0.5) {
		...
		//schapen plaatsen
		GameObject Schaap = Resources.Load ("Schaapprefab 1") as GameObject;
		for(int i = 0; i < aantalSchapen; i++){
			float posx = Random.Range(beginx,eindx);
			float posz = Random.Range(beginz,eindz);
			if (watercheck (waterx, waterz, waterr, posz, posx) ==2){
				GameObject schaap = Instantiate(Schaap);
				schaap.tag = "Schaap";
				schaap.transform.position = new Vector3 (posx, Terrain.activeTerrain.SampleHeight(new Vector3(posx,0,posz)) 
			                                         + Terrain.activeTerrain.transform.position.y, posz);
				schaap.transform.localScale = new Vector3(3,3,3);
			}
		}
		//plaats schuur
		GameObject Schuur = Resources.Load ("Barnmodel") as GameObject;
		GameObject schuur = Instantiate (Schuur) as GameObject;
		schuur.transform.position = new Vector3 ((float)(beginx + eindx) / 2, Terrain.activeTerrain.SampleHeight (new Vector3 (beginx, 0, beginz))-0.1f 
		                                         + Terrain.activeTerrain.transform.position.y, eindz*0.99f);
		schuur.transform.eulerAngles = new Vector3 (0,180,0);
		schuur.transform.localScale = new Vector3 (5, 5, 5);
